...
## Read parquet from S3 using ObservableJS

::: {#map}

```{ojs}
//| label: read-s3-ojs
//| libraries: [leaflet, apache-arrow]

// Load libraries in Observable
L = require("leaflet");
arrow = require("apache-arrow");

// Load the geoarrow-js library explicitly
geoarrow = await import("https://cdn.jsdelivr.net/npm/@geoarrow/geoarrow-js@0.3.2/dist/geoarrow.umd.min.js");
...
// Create map
map = L.map("map").setView([35.5, -80], 7);
...
// Function to load parquet and display on map
async function loadParquet() {
  try {
    ...
    // Filter counties starting with "A" and select NAME and geom columns
    const filteredTable = table.filter(
      arrow.predicate.custom(i => table.getColumn("NAME").get(i).startsWith("A"))
    ).select(["NAME", "geom"]);

    // Convert arrow geometry to GeoJSON
    const features = [];
    const nameColumn = filteredTable.getColumn("NAME");
    const geomColumn = filteredTable.getColumn("geom");

    // Get the geometry array from geoarrow
    const geometries = geoarrow.getGeometryArray(geomColumn);

    for (let i = 0; i < filteredTable.numRows; i++) {
    ...
    return `Error loading map: ${error.message}`;
  }
}
