...
```{ojs}
//| label: read-s3-js-setup
//| output: false

// Add JavaScript dependencies using URLs instead of require
// This approach avoids the package.json loading issue

// Apache Arrow
apacheArrowScript = html`<script src="https://cdn.jsdelivr.net/npm/apache-arrow@14.0.2/Arrow.es2015.min.js"></script>`

// D3.js
d3Script = html`<script src="https://cdn.jsdelivr.net/npm/d3@7.8.5/dist/d3.min.js"></script>`

// Leaflet
leafletScript = html`<script src="https://cdn.jsdelivr.net/npm/leaflet@1.9.4/dist/leaflet.js"></script>`
leafletCSS = html`<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/leaflet@1.9.4/dist/leaflet.css">`

// GeoArrow
geoarrowScript = html`<script src="https://cdn.jsdelivr.net/npm/geoarrow-js@0.2.1/dist/geoarrow.umd.js"></script>`
```
...
mapData = {
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      // Convert to Arrow table using global Arrow object
      const table = await window.Arrow.Table.from(new Uint8Array(buffer))
      ...
      const filtered = table.select(table.schema.fields.map(f => f.name))
                           .filter(window.d3.range(table.numRows).map(i => indices.includes(i)))

      // Convert to GeoJSON using geoarrow
      let geoJSON
      try {
        // Access the geoarrow module functions from global scope
        geoJSON = window.geoarrow.tableToGeoJSON(filtered)
      } catch (error) {
